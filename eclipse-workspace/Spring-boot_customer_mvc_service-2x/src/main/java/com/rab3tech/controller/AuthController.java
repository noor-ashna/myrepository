...
public class AuthController {
	...
	public String changePasswordPost(int sid,String newPassword,
		...
		signupService.updatePassword(sid,newPassword);
		return "redirect:/auth?message=Your password is updated successfully";
	}
	
	@GetMapping("changePassword")
	public String showChangePasswordPage(@RequestParam("sid") int sid,Model model) {
		//Below is spring jdbc programming
		SignUp signUp=signupService.findById(sid);
		//Adding data into request scope
		model.addAttribute("signUp", signUp);
		return "chpassword";// chpassword.jsp
	}
	...
	//This action will show all the signups data over jsp
		...
		public String updateDogs(HttpServletRequest request,Model model) {
			//Below code will delete the record as sid passed from html page
			String sid = request.getParameter("sid");
			signupService.delete(sid);
			//below is redirecting request to the new action
